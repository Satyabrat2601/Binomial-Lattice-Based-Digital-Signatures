...
static const int32_t zetas[DUET_N] = {
   ...
   -17737, -22805,  -3528,     65,  10770,   8908, -23751,  26934,
    ...
    13803,  -8099,   2931,   9217, -21126, -14203,  25492, -12831,
     7947,  17463, -12979,  29003,  31612,  26554,   8241, -20175
};
...
void ntt(int32_t a[DUET_N]) {
    ...
    for (len = 128; len > 0; len >>= 1) {
        for (start = 0; start < DUET_N; start = j + len) {
            zeta = zetas[++k];
...
/* Inverse NTT multiplied by 2^32 (Gentleman-Sande, natural order output) */
void invntt_tomont(int32_t a[DUET_N]) {
    unsigned int start, len, j, k;
    int32_t t, zeta;
    const int32_t f = -29720; /* MONT^2 / 256 mod q */
    k = 256;
    for (len = 1; len < DUET_N; len <<= 1) {
        for (start = 0; start < DUET_N; start = j + len) {
            zeta = -zetas[--k];
            for (j = start; j < start + len; ++j) {
                t          = a[j];
